...
def process_regions():
    ...
    for region_id, region_hull in region_hulls.iterrows():
        ...
        if not tesselation_coverage.all():
            ...
            print('Retrying tesselation with less buildings, potentially changing building data.')
            ...
            if tesselation_coverage.all():
                print('Dropping', num_problem_buildings, 'buildings due to tesselation problems')
        ...
        ## save files
        tesselations.to_parquet(data_dir + f'tessellations/tessellation_{region_id}.parquet')
...
def generate_tess(buildings, enclosures, n_workers=1):
    ...
    tessellation = mm.enclosed_tessellation(buildings,
                                            enclosures.geometry,
                                            simplify=True,
                                            n_jobs=n_workers)
    ...
    to_keep = np.append(np.unique(inp), 
                        np.where(tessellation.index.values < 0)[0])
...
def generate_enclosures(buildings, streets):
    ...
    gabriel = Graph.build_triangulation(buildings.representative_point(), "gabriel", kernel="identity")
    ...
    enclosures = mm.enclosures(streets, 
                               limit = buffered_buildings)
    return enclosures
