...
def partial_describe_reached_agg(y, graph_index, graph, higher_order, n_splits, q=None, statistics=None):
    ...
    for partial_higher in lazy_higher_order(graph, k=higher_order, n_splits=n_splits):
        ...
        relevant_groups = np.intersect1d(
            partial_higher.unique_ids, reverse_graph_index.index.values, assume_unique=True
        )
...
def pprint_cluster_percentiles(X_train, labels):
    ...
    f = {k :'{:.4f}' for k in cluster_stats.columns.values} #column col A to 2 decimals
...
used_keys = {'sdbAre': 'area of building',
 'sdbPer': 'perimeter of building',
 'sdbCoA': 'courtyard area of building',
 'ssbCCo': 'circular compactness of building',
 'ssbCor': 'corners of building',
