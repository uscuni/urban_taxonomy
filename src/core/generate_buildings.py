...
def process_regions(largest):
    ...
    if largest:
        ...
        new = Parallel(n_jobs=n_jobs)(
            delayed(process_single_region_buildings)(region_id)
...
def process_single_region_buildings(region_id):
    ...
    buildings = process_region_buildings(
        buildings, True, simplification_tolerance=0.1, merge_limit=25
    )
...
def process_region_buildings(
    buildings, simplify, simplification_tolerance=0.1, merge_limit=25
):
...
def drop_buildings_intersecting_streets(buildings, region_id):
    ...
    ## drop tunnels
    to_filter = (
        streets.loc[~streets.road_flags.isna(),].set_crs(epsg=4236).to_crs(epsg=3035)
    )
    ...
    blg_idxs, str_idxs = streets.buffer(0.3).sindex.query(
        buildings.geometry.to_crs(epsg=3035), predicate="intersects"
    )
