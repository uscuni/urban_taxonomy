...
def process_tessellation_graph(region_id, graph_dir, tessellations_dir):
    ...
    graph.to_parquet(graph_dir + f"tessellation_graph_{region_id}.parquet")
...
def process_buildings_graph(region_id, graph_dir, buildings_dir):
    ...
    graph = Graph.build_fuzzy_contiguity(buildings, buffer=0.25).assign_self_weight()

    graph.to_parquet(graph_dir + f"building_graph_{region_id}.parquet")
...
def process_edges_graph(region_id, graph_dir, streets_dir):
    ...
    graph.to_parquet(graph_dir + f"street_graph_{region_id}.parquet")
...
def process_enclosure_graph(region_id, graph_dir, enclosures_dir):
    ...
    graph.to_parquet(graph_dir + f"enclosure_graph_{region_id}.parquet")
...
def process_nodes_graph(region_id, graph_dir, streets_dir):
    ...
    graph.to_parquet(graph_dir + f"nodes_graph_{region_id}.parquet")
